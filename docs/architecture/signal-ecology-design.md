...
### 8.7 Passive Recon Integration

Radio intercept chunks generated by the Passive Recon system now incorporate
the Signal Ecology composite value at capture time. The `signalMod` field on
radio intercept and satellite intercept chunks includes both the device tier
confidence AND a BPS bonus/penalty derived from the live composite signal.

The scaling factor (`SIGNAL_ECOLOGY_BPS_SCALE = 4000`) maps the composite
deviation from the 0.5 midpoint to BPS:
- At composite 0.85 (locked): +1400 BPS bonus
- At composite 0.50 (faded): +0 BPS
- At composite 0.10 (ghosted): -1600 BPS penalty

Building scan chunks (camcorder, field logger) are **not** affected by
Signal Ecology because they use visual detection, not RF propagation.

Each radio/satellite chunk also captures the `signalState` field at
generation time for diagnostic display on terminal screens.
